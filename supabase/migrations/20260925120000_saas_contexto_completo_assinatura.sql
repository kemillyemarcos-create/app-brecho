...
declare
  ...
  v_recursos jsonb;
  v_ultima_assinatura_historica jsonb;
begin
  ...
  if not found then
    -- Histórico separado: nunca substitui a assinatura corrente nem libera acesso.
    -- created_at identifica a assinatura criada mais recentemente; updated_at pode
    -- mudar por manutenção e canceled_at pode registrar término contratual passado.
    -- id desempata deterministicamente assinaturas criadas no mesmo instante.
    select jsonb_build_object(
      ...
      'current_period_started_at', a.current_period_started_at,
      'current_period_ends_at', a.current_period_ends_at,
      'billing_anchor_at', a.billing_anchor_at,
      ...
      into v_ultima_assinatura_historica
    from public.assinaturas a
    where a.empresa_id = p_empresa_id
    ...
    return jsonb_build_object(
      ...
      'acesso_operacional', public.assinatura_empresa_operacional_ativa(p_empresa_id),
      'possui_assinatura', false,
      'assinatura', null,
      'ultima_assinatura_historica', v_ultima_assinatura_historica,
      'plano', null,
      'recursos', '{}'::jsonb
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'ultima_assinatura_historica', null,
    'assinatura', jsonb_build_object(
      ...
      'billing_anchor_at', v_assinatura.billing_anchor_at,
      'grace_period_started_at', v_assinatura.grace_period_started_at,
      'grace_period_ends_at', v_assinatura.grace_period_ends_at,
