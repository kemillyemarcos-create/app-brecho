-- Sem default ou backfill: registros sem origem comprovada permanecem NULL.
alter table public.assinaturas
add column billing_anchor_at timestamptz;

comment on column public.assinaturas.billing_anchor_at
is 'Referência comercial original em UTC. Não é o período corrente; permanece estável nas renovações. Nula para registros sem billing definido.';

create or replace function public.calcular_fim_periodo_assinatura(
...
declare
  ...
  v_passo integer;
begin
  if p_anchor_at is null or p_periodo_inicio is null
     or p_periodicidade is null then
    raise exception 'Âncora, início e periodicidade são obrigatórios.'
      using errcode = '22004';
  end if;

  if not isfinite(p_anchor_at) or not isfinite(p_periodo_inicio) then
    raise exception 'Datas comerciais devem ser finitas.' using errcode = '22023';
  end if;

  if p_periodicidade not in ('mensal', 'anual') then
    raise exception 'Periodicidade inválida.' using errcode = '22023';
  end if;

  v_anchor := p_anchor_at at time zone 'UTC';
  v_inicio := p_periodo_inicio at time zone 'UTC';
  v_passo := case p_periodicidade when 'mensal' then 1 else 12 end;
  v_meses := (extract(year from v_inicio)::integer - extract(year from v_anchor)::integer) * 12
    + extract(month from v_inicio)::integer - extract(month from v_anchor)::integer;

  -- Cada limite deriva da origem, nunca do limite anterior já truncado.
  -- PostgreSQL ajusta dias inexistentes ao último dia do mês de destino.
  -- Preserva também hora e frações de segundo no calendário UTC.
  if v_meses < 0 or mod(v_meses, v_passo) <> 0
     or v_inicio <> v_anchor + make_interval(months => v_meses) then
    raise exception 'Início do ciclo incompatível com a âncora comercial.'
      using errcode = '22023';
  end if;

  return (v_anchor + make_interval(months => v_meses + v_passo)) at time zone 'UTC';
end;
$$;

revoke all on function public.calcular_fim_periodo_assinatura(timestamptz, timestamptz, text) from public;
revoke all on function public.calcular_fim_periodo_assinatura(timestamptz, timestamptz, text) from anon;
revoke all on function public.calcular_fim_periodo_assinatura(timestamptz, timestamptz, text) from authenticated;
grant execute on function public.calcular_fim_periodo_assinatura(timestamptz, timestamptz, text) to service_role;
